...
public class DownloadGui extends WindowScreen {
    ...
    private ConfirmationModalBuilder makeModal() {
        ...
        builder.setSecondaryText("(This will update from v" + Tabulous.VER + " to " + Updater.latestTag + ")");
        builder.setOnConfirm((wyvest) -> {
            ...
            Multithreading.runAsync(() -> {
                if (Updater.download(
                        Updater.updateUrl,
                        new File(
                                "mods/" + Tabulous.NAME + "-" + Updater.latestTag.substring(Updater.latestTag.indexOf("v")) + ".jar"
                        )
                ) && Updater.download(
                        "https://github.com/Wyvest/Deleter/releases/download/v1.2/Deleter-1.2.jar",
                        new File(Tabulous.modDir.getParentFile(), "Deleter-1.2.jar")
                )
                ) {
                    EssentialAPI.getNotifications()
                            .push(Tabulous.NAME, "The ingame updater has successfully installed the newest version.");
